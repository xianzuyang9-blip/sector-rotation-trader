...
from crazy.adapters.rss_count import fetch_rss_counts


class DailySurgeInRssNewsCountsOnCorporateDividendIncreasAlgo(CrazyAlgoBase):
    algo_id = "daily-surge-in-rss-news-counts-on-corporate-dividend-increas"
    name = "Daily Surge In Rss News Counts On Corporate Dividend Increases Signals Financial Sector Strength"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_rss_counts(feed_urls=['https://news.google.com/rss/search?q=Daily+Surge+In+Rss+News+Counts+On+Corporate+Dividend+Increases+Signals+Financial+Sector+Strength&hl=en-US&gl=US&ceid=US:en'], days_back=180)
        ...
        if held:
            should_exit = days_held >= 5
            if "up" == "up":
                should_exit = should_exit or change < (1.3 / 2.0)
            else:
                should_exit = should_exit or change > -(1.3 / 2.0)
            if should_exit:
