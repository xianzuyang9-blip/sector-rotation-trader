...
class WeeklySpikeInRssNewsCountsMentioningCorporateBondDoAlgo(CrazyAlgoBase):
    algo_id = "weekly-spike-in-rss-news-counts-mentioning-corporate-bond-do"
    name = "Weekly Spike In Rss News Counts Mentioning Corporate Bond Downgrade Signals Financial Stress"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_rss_counts(feed_urls=['https://news.google.com/rss/search?q=corporate+bond+downgrade&hl=en-US&gl=US&ceid=US:en'], days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.5 / 2.0)
            else:
                should_exit = should_exit or change > -(0.5 / 2.0)
            if should_exit:
