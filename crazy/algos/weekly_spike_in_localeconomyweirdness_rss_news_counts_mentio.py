...
class WeeklySpikeInLocaleconomyweirdnessRssNewsCountsMentioAlgo(CrazyAlgoBase):
    algo_id = "weekly-spike-in-localeconomyweirdness-rss-news-counts-mentio"
    name = "Weekly Spike In Localeconomyweirdness Rss News Counts Mentioning Small Town Retail Closures Signals Consumer Discretionary Softness"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_rss_counts(feed_urls=['https://news.google.com/rss/search?q=small+town+retail+closures&hl=en-US&gl=US&ceid=US:en'], days_back=180)
        ...
        if held:
            should_exit = days_held >= 28
            if "up" == "up":
                should_exit = should_exit or change < (0.3 / 2.0)
            else:
                should_exit = should_exit or change > -(0.3 / 2.0)
            if should_exit:
