...
class HealthcareCostSearchSpikeSignalsStaplesDefensiveRotatAlgo(CrazyAlgoBase):
    algo_id = "healthcare-cost-search-spike-signals-staples-defensive-rotat"
    name = "Healthcare Cost Search Spike Signals Staples Defensive Rotation"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='health insurance deductible', days_back=180)
        ...
        if held:
            should_exit = days_held >= 9
            if "up" == "up":
                should_exit = should_exit or change < (0.45 / 2.0)
            else:
                should_exit = should_exit or change > -(0.45 / 2.0)
            if should_exit:
