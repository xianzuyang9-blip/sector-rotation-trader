...
class PortCongestionNewsSpikeSignalsLogisticsHeadwindReversAlgo(CrazyAlgoBase):
    algo_id = "port-congestion-news-spike-signals-logistics-headwind-revers"
    name = "Port Congestion News Spike Signals Logistics Headwind Reversal Trade"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.6 / 2.0)
            else:
                should_exit = should_exit or change > -(0.6 / 2.0)
            if should_exit:
