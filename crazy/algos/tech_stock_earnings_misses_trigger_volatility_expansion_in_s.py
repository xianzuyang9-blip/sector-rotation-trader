...
class TechStockEarningsMissesTriggerVolatilityExpansionInSAlgo(CrazyAlgoBase):
    algo_id = "tech-stock-earnings-misses-trigger-volatility-expansion-in-s"
    name = "Tech Stock Earnings Misses Trigger Volatility Expansion In Semiconductor Holdings"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_rss_counts(feed_urls=['https://news.google.com/rss/search?q=earnings+miss&hl=en-US&gl=US&ceid=US:en'], days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (1.5 / 2.0)
            else:
                should_exit = should_exit or change > -(1.5 / 2.0)
            if should_exit:
