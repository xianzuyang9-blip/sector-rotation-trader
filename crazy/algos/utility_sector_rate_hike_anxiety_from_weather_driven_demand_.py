...
class UtilitySectorRateHikeAnxietyFromWeatherDrivenDemandAlgo(CrazyAlgoBase):
    algo_id = "utility-sector-rate-hike-anxiety-from-weather-driven-demand-"
    name = "Utility Sector Rate Hike Anxiety From Weather-driven Demand Spike"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        if held:
            should_exit = days_held >= 6
            if "up" == "up":
                should_exit = should_exit or change < (0.9 / 2.0)
            else:
                should_exit = should_exit or change > -(0.9 / 2.0)
            if should_exit:
