...
class EvChargerInstallationBoomAccelerationSignalAlgo(CrazyAlgoBase):
    algo_id = "ev-charger-installation-boom-acceleration-signal"
    name = "Ev Charger Installation Boom Acceleration Signal"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.025 / 2.0)
            else:
                should_exit = should_exit or change > -(0.025 / 2.0)
            if should_exit:
