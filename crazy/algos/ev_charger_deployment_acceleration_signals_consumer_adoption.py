...
class EvChargerDeploymentAccelerationSignalsConsumerAdoptionAlgo(CrazyAlgoBase):
    algo_id = "ev-charger-deployment-acceleration-signals-consumer-adoption"
    name = "Ev Charger Deployment Acceleration Signals Consumer Adoption Surge"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.03 / 2.0)
            else:
                should_exit = should_exit or change > -(0.03 / 2.0)
            if should_exit:
