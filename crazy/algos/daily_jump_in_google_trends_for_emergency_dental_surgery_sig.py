...
class DailyJumpInGoogleTrendsForEmergencyDentalSurgerySigAlgo(CrazyAlgoBase):
    algo_id = "daily-jump-in-google-trends-for-emergency-dental-surgery-sig"
    name = "Daily Jump In Google Trends For Emergency Dental Surgery Signals Healthcare Consumer Stress"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='Daily Jump In Google Trends For Emergency Dental Surgery Signals Healthcare Consumer Stress', days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (1.2 / 2.0)
            else:
                should_exit = should_exit or change > -(1.2 / 2.0)
            if should_exit:
