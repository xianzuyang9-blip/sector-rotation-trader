...
class DailySpikeInGoogleTrendsForDiyHomeSecurityInstallatAlgo(CrazyAlgoBase):
    algo_id = "daily-spike-in-google-trends-for-diy-home-security-installat"
    name = "Daily Spike In Google Trends For Diy Home Security Installation Signals Consumer Tech Caution"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='DIY home security installation', days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.25 / 2.0)
            else:
                should_exit = should_exit or change > -(0.25 / 2.0)
            if should_exit:
