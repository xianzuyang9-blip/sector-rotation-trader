...
class DailySpikeInGoogleTrendsForGasStationCashOnlySearcAlgo(CrazyAlgoBase):
    algo_id = "daily-spike-in-google-trends-for-gas-station-cash-only-searc"
    name = "Daily Spike In Google Trends For Gas Station Cash Only Searches Indicates Consumer Payment Stress"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='gas station cash only', days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.35 / 2.0)
            else:
                should_exit = should_exit or change > -(0.35 / 2.0)
            if should_exit:
