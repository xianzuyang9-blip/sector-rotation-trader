...
class WeeklySurgeInGoogleTrendsForWarehouseAutomationJobsAlgo(CrazyAlgoBase):
    algo_id = "weekly-surge-in-google-trends-for-warehouse-automation-jobs-"
    name = "Weekly Surge In Google Trends For Warehouse Automation Jobs Signals Industrial Tech Adoption"
    family = "labor_jobs"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='Weekly Surge In Google Trends For Warehouse Automation Jobs Signals Industrial Tech Adoption', days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (1.25 / 2.0)
            else:
                should_exit = should_exit or change > -(1.25 / 2.0)
            if should_exit:
