...
class WarehouseRoboticsNewsSurgeSignalsIndustrialTechDemandAlgo(CrazyAlgoBase):
    algo_id = "warehouse-robotics-news-surge-signals-industrial-tech-demand"
    name = "Warehouse Robotics News Surge Signals Industrial Tech Demand Inflection"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_rss_counts(feed_urls=['https://news.google.com/rss/search?q=warehouse+robotics&hl=en-US&gl=US&ceid=US:en'], days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.55 / 2.0)
            else:
                should_exit = should_exit or change > -(0.55 / 2.0)
            if should_exit:
