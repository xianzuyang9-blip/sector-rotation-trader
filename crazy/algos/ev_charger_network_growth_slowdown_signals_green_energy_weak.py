...
class EvChargerNetworkGrowthSlowdownSignalsGreenEnergyWeakAlgo(CrazyAlgoBase):
    algo_id = "ev-charger-network-growth-slowdown-signals-green-energy-weak"
    name = "Ev Charger Network Growth Slowdown Signals Green Energy Weakness"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        if held:
            ...
            if "down" == "up":
                should_exit = should_exit or change < (0.02 / 2.0)
            else:
                should_exit = should_exit or change > -(0.02 / 2.0)
            if should_exit:
