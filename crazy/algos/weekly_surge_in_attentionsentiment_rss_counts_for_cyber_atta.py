...
class WeeklySurgeInAttentionsentimentRssCountsForCyberAttaAlgo(CrazyAlgoBase):
    algo_id = "weekly-surge-in-attentionsentiment-rss-counts-for-cyber-atta"
    name = "Weekly Surge In Attentionsentiment Rss Counts For Cyber Attack On Utilities Signals Energy Sector Risk"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_rss_counts(feed_urls=['https://news.google.com/rss/search?q=cyber+attack+on+utilities&hl=en-US&gl=US&ceid=US:en'], days_back=180)
        ...
        if held:
            should_exit = days_held >= 21
            if "up" == "up":
                should_exit = should_exit or change < (0.4 / 2.0)
            else:
                should_exit = should_exit or change > -(0.4 / 2.0)
            if should_exit:
