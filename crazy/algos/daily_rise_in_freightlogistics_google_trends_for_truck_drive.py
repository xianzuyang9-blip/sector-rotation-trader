...
class DailyRiseInFreightlogisticsGoogleTrendsForTruckDriveAlgo(CrazyAlgoBase):
    algo_id = "daily-rise-in-freightlogistics-google-trends-for-truck-drive"
    name = "Daily Rise In Freightlogistics Google Trends For Truck Driver Shortage Signals Industrial Sector Labor Supply Constraint"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='truck driver shortage', days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.15 / 2.0)
            else:
                should_exit = should_exit or change > -(0.15 / 2.0)
            if should_exit:
