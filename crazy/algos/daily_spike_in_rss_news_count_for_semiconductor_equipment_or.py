...
class DailySpikeInRssNewsCountForSemiconductorEquipmentOrAlgo(CrazyAlgoBase):
    algo_id = "daily-spike-in-rss-news-count-for-semiconductor-equipment-or"
    name = "Daily Spike In Rss News Count For Semiconductor Equipment Orders Signals Chip Sector Rebound"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_rss_counts(feed_urls=['https://news.google.com/rss/search?q=Daily+Spike+In+Rss+News+Count+For+Semiconductor+Equipment+Orders+Signals+Chip+Sector+Rebound&hl=en-US&gl=US&ceid=US:en'], days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (1.4 / 2.0)
            else:
                should_exit = should_exit or change > -(1.4 / 2.0)
            if should_exit:
